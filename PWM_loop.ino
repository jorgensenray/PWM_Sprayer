...
void PWM_loop() {
  ...
  if (On_Off == 0) {              // Main on off switch 1 = on
    Flow();                       // Flow sensor
...
void loadUserSettingsFromEEPROM() {
  UserSettings tempSettings;  // Temporary struct to check if data exists
  EEPROM.get(MY_EEPROM_ADDR, tempSettings);

  // Check if EEPROM contains valid data (simple example check)
  if (tempSettings.GPATarget > 0 && tempSettings.GPATarget < 20) {
    userSettings = tempSettings;
    Serial.println(" ");
    Serial.println(" User variables loaded from EEPROM. ");

  } else {
    Serial.println("No valid user settings found, using defaults.");
...
void saveuserSettingsToEEPROM() {
  EEPROM.put(MY_EEPROM_ADDR, userSettings);
